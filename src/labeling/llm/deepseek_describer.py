...
class DeepSeekDescriber(CharacterDescriber):
    ...
    def _complete(
        ...
        if appearance_text:
            lines.append(
                "Теги внешности с изображения (только чтобы опознать персонажа, "
                f"не пересказывать): {appearance_text}."
            )
        if wiki_url:
    ...
    def _system_prompt(self) -> str:
        return (
            ...
            "Тебе дают имя персонажа или реального человека, категорию, из которой он взят, "
            "и, если есть, теги внешности с изображения карточки, ссылку на страницу персонажа "
            "и заметки.\n\n"
            "Шаг 1 — определи, кто это. Имя может быть неоднозначным: тёзки встречаются "
            "в разных франшизах. Сверяйся с категорией (она задаёт тип источника — аниме, фильм, "
            "сериал, музыка, индустрия для взрослых и т.п. — и ей нельзя противоречить) и с тегами "
            "внешности (пол, возраст, тип персонажа, приметы). Если твоё первое предположение "
            "противоречит категории или тегам — это другой персонаж, пересмотри выбор.\n\n"
            "Теги внешности нужны ТОЛЬКО для опознания персонажа. ЗАПРЕЩЕНО пересказывать их "
            "в описании и фактах: игрок видит карточку и сам знает, какие у персонажа волосы, "
            "глаза, одежда, поза и выражение лица. Описание внешности с картинки — бесполезный "
            "материал.\n\n"
            "Верни строго JSON с ключами description и facts без лишнего текста.\n\n"
            "description — описание в стиле статьи из википедии на русском языке "
            "(2-4 предложения). Игрок читает его приватно вместе с карточкой, чтобы понять, "
            "кто ему достался. Обязательно: кто это и ОТКУДА — название фильма, сериала, аниме, "
            "игры, группы или индустрии указывай явно («персонаж сериала „Во все тяжкие“», "
            "«вокалист группы Nirvana», «американская порноактриса»). Дальше — чем занимается, "
            "какова его роль в сюжете, чем известен. Имя и франшизу в description упоминать "
            "МОЖНО и нужно. Пример хорошего описания: «Джина Валентина — американская "
            "порноактриса и эротическая модель. Она начала свою карьеру в индустрии для "
            "взрослых в 2013 году и с тех пор снялась в многочисленных фильмах.»\n\n"
            f"facts — ровно {self.facts_count} коротких фактов на русском языке от третьего "
            "лица. Игрок зачитывает факт вслух как подсказку о своём персонаже, поэтому факты "
            "должны звучать как реплики живого игрока: биография, род занятий, поступки, "
            "характер, отношения, привычки, коронные фразы, судьба. "
            "Пример хороших фактов: «Бросил школу, но стал партнёром своего бывшего учителя», "
            "«Разбирается в химии лучше, чем кажется», «Потерял близкого человека из-за своей "
            "работы». "
            "В фактах ЗАПРЕЩЕНО: упоминать имя персонажа, его прозвища и название "
            "франшизы/произведения (это мгновенно раскрывает карточку); описывать внешность "
            "и то, что видно на изображении (причёска, цвет глаз, одежда, телосложение, поза).\n\n"
            "Не выдумывай: факты должны быть конкретными и проверяемыми. Если персонаж "
            "малоизвестен и ты не уверен в деталях — пиши только то, что знаешь достоверно, "
            "опираясь на категорию, и не сочиняй биографию."
        )
